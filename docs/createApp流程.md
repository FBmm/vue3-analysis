...
**创建 vue 应用实例的核心逻辑在 renderer 对象的 createApp 方法中实现**

ensureRenderer 初始化 renderer 对象时，会调用 createAppAPI 方法返回 createApp 方法，createAppAPI是一个闭包函数，因为 vue 实例 mount 需要调用 render 函数。
